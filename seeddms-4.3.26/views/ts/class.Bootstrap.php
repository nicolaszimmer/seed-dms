...
';*/
	...
	function getFolderPathHTML($folder, $tagAll=false, $document=null) { /* {{{ */
		$path = $folder->getPath();
		$txtpath = "<li><strong>".getMLText($document?"document":"folder")."</strong> <i class='icon-caret-right divider'></i></li>";
...
//		echo "<div class=\"contentSubHeading\"".($first ? " id=\"first\"" : "").">".htmlspecialchars($heading)."</div>\n";
	...
	function getMimeIcon($fileType) { /* {{{ */
		...
		$icons = array();
		$icons["txt"]  = "icon-file-text-alt";
		$icons["text"] = "icon-file-text-alt";
		$icons["doc"]  = "icon-file-text-alt";
		$icons["dot"]  = "icon-file-text-alt";
		$icons["docx"] = "icon-file-text-alt";
		$icons["dotx"] = "icon-file-text-alt";
		$icons["rtf"]  = "icon-file-text-alt";
		$icons["pages"]  = "icon-file-text-alt";
		$icons["xls"]  = "icon-table";
		...
		$icons["htm"]  = "icon-code";
		$icons["js"]  = "icon-code";
		$icons["gif"]  = "icon-picture";
		$icons["jpg"]  = "icon-picture";
		$icons["jpeg"] = "icon-picture";
		$icons["bmp"]  = "icon-picture";
		$icons["png"]  = "icon-picture";
		$icons["tif"]  = "icon-picture";
		$icons["tiff"] = "icon-picture";
		$icons["log"]  = "icon-file-text-alt";
		$icons["pdf"]  = "icon-file-text-alt";
		$icons["wav"]  = "icon-volume-up";
		$icons["mp3"]  = "icon-volume-up";
		$icons["tar"]  = "icon-archive";
		$icons["gz"]   = "icon-archive";
		$icons["7z"]   = "icon-archive";
		$icons["bz"]   = "icon-archive";
		$icons["bz2"]  = "icon-archive";
		$icons["tgz"]  = "icon-archive";
		$icons["zip"]  = "icon-archive";
		$icons["rar"]  = "icon-archive ";
		$icons["mpg"]  = "icon-film";
		$icons["avi"]  = "icon-film";
		$icons["mp4"]  = "icon-film";
		$icons["mov"]  = "icon-film";
		...
		if($latestContent = $document->getLatestContent()) {
			...
			if (file_exists($dms->contentDir . $latestContent->getPath())) {
				...
				if($previewer->hasPreview($latestContent)) {
					...
					$content .= "<i draggable=\"false\" class=\"".$this->getMimeIcon($latestContent->getFileType())." larger\" title=\"".htmlspecialchars($latestContent->getMimeType())."\"></i>";
				}
				$content .= "</a></td>";
			} else
				$content .= "<td><img draggable=\"false\" class=\"mimeicon\" src=\"".$this->getMimeIcon($latestContent->getFileType())."\" title=\"".htmlspecialchars($latestContent->getMimeType())."\"></td>";
...
//		$folder = $this->params['folder'];
	...
	//	$content .= "<td><img src=\"images/folder_closed.gif\" width=18 height=18 border=0></td>";
		$content .= "<td><a class='transient' _rel=\"folder_".$subFolder->getID()."\" draggable=\"false\" href=\"out.ViewFolder.php?folderid=".$subFolder->getID()."&showtree=".$showtree."\"><i class='icon-folder-open-alt larger'></i></a></td>\n";
